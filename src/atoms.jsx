...
const TriMark = ({ size = 10, color = "currentColor" }) => (
  <span
    className="tri-mark"
    aria-hidden
    style={{
...
function OrbitalCanvas({ playing }) {
  ...
  useEffect(() => {
    ...
    const orbs = [
      { r: 0.30, speed: 0.0005, phase: 0, sz: 3.0, c: "oklch(0.84 0.14 210)" },
      { r: 0.30, speed: 0.0005, phase: Math.PI * 0.9, sz: 2.2, c: "oklch(0.84 0.14 210 / 0.6)" },
      { r: 0.42, speed: -0.0003, phase: 1.4, sz: 3.6, c: "oklch(0.82 0.14 75)" },
      { r: 0.42, speed: -0.0003, phase: 3.8, sz: 2.0, c: "oklch(0.82 0.14 75 / 0.6)" },
      { r: 0.50, speed: 0.0002, phase: 2.2, sz: 2.8, c: "oklch(0.7 0.18 290)" }
    ];
    ...
    const draw = () => {
      ...
      for (const o of orbs) {
        ...
        for (let i = 1; i < 12; i++) {
          ...
          ctx.beginPath(); ctx.arc(xx, yy, o.sz * (1 - i / 14), 0, Math.PI * 2); ctx.fill();
        }
        ctx.fillStyle = o.c;
...
Object.assign(window, { Chip, Kbd, TriMark, Wordmark, EqBars, OrbitalCanvas });
